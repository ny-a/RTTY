...
def read_wave_file(filename, mark_frequency = 914, space_frequency = 1086):
    with wave.open(filename, 'rb') as wave_file:
        ...
        for j in range(wave_file.getnframes()):
            ...
            mark_q[array_index] = int((buf[0]-128) * np.sin(np.pi * 2.0 / sampling_rate * mark_frequency * j))
            mark_i[array_index] = int((buf[0]-128) * np.cos(np.pi * 2.0 / sampling_rate * mark_frequency * j))
            space_q[array_index] = int((buf[0]-128) * np.sin(np.pi * 2.0 / sampling_rate * space_frequency * j))
            space_i[array_index] = int((buf[0]-128) * np.cos(np.pi * 2.0 / sampling_rate * space_frequency * j))
